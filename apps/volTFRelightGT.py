...
if __name__ == "__main__":
    ...
    parser.add_argument('-m','--mode', type=str, default='train', help='Train mode, test mode (circle render path), valid mode (nu=2)')
    parser.add_argument('--TF', type=int, default=-1, help='The index of TF to render, -1 to render all TFs')
    ...
    len_cam_views = len(all_cam_theta_phi)
    test_cam_views = [all_cam_theta_phi[len_cam_views//2]]
    ...
    for t in timesteps: #* here we render all timesteps
        ...
        for TF_count, TF in enumerate(allTFs):
            if TF_idx != -1 and TF_count+1 != TF_idx:
                continue
            #* setup RootImgDir for each TF
            TFRootDir = args.outImgDir + f'/TF{TF_count+1}'
            render.setup(TFRootDir)
            write_TF_json(TF, f'{TFRootDir}/TF{TF_count+1}.npy')
            ...
            #* render each view
            for cam_theta_phi in test_cam_views:
                for idx, (light_theta_phi) in enumerate(all_light_theta_phi):
